...
public class Online_CDAWG_sym extends IndexStructure {
	...
	private void add_suffixlink(Node node) {
		...
		if (suffixstate != null) {
			...
			create_edge_left(node, suffixstate,
					this.get_letter(suffixstate.end - this.get_node_length(node), suffixstate.stringnr)); // create
																											// left_edge
																											// from
																											// split to
																											// next
	...
	private void update() {
		...
		while (true) {
			...
			if (!(has_outgoing_edge(ap.active_node, ap.active_edge))) { // if no edge with label of
				...
				if (active_child == ap.active_node.children.get(ap.active_edge)) {
					...
					if (ap.active_node == root) {
					...
					} else if (this.get_edge_length(ap.active_edge, ap.active_node, split)
							+ this.get_node_length(ap.active_node) < this.get_node_length(split)) {

						// länge start knoten + läng der kante < länge des zielknotens
						int left_letter_pos = split.start + this.get_node_length(split)
						...
						if (print)
							System.out.println(this.get_node_label(ap.active_node) + " pos= " + left_letter_pos + " "
									+ this.get_letter_by_idx(active_edge_left) + " " + split.stringnr);
						if (print)
							System.out.println(active_edge_left);
						if (print)
							System.out.println(this.get_node_label(next_left));

						if (this.get_edge_label_left(active_edge_left, ap.active_node, next_left).length()
								+ this.get_node_length(ap.active_node) < this.get_node_length(next_left)) {
							redirect_edge_left(ap.active_node, split, active_edge_left);
						}
					}
				...
				// Split
					...
					create_edge(split, next, this.get_letter(last_char_occ, next.stringnr)); // create edge from split
																								// to next
					...
					if (this.get_node_length(ap.active_node) + active_label.length() == this.get_node_length(next)) {
						...
						if (print)
							System.out.println(this.get_letter(split.start - 1, next.stringnr));
						create_edge_left(split, next, this.get_letter(split.start - 1, next.stringnr)); // create edge
																										// from split to
																										// next
					}
					...
					if (old_suffixstate == null || old_suffixstate.suffixLink != split) {
						...
						if (print)
							System.out.println("STELLE left cahr occ " + left_char_occ + "  "
									+ ((ap.active_length + this.get_node_length(ap.active_node)) - 1));

						if (split.start != 0)
							create_edge_left(split, sink, this.get_letter(left_char_occ, sink.stringnr)); // create
																											// left_edge
																											// from
																											// split to
																											// sink
		...
		if (target_ap != null) {
			if (print)
				System.out.println(this.get_node_label(target_ap) + " " + ap.active_length);

			if (this.get_edge_length(ap.active_edge, ap.active_node, target_ap) == ap.active_length
					& !this.get_node_label(target_ap).startsWith("#")) {

				int left_char_occ = pos - this.get_node_length(target_ap);

				if (print)
					System.out.println("LEFT CHAR OCC " + left_char_occ + " " + sink.stringnr);
	...
	void separate_node() {
		...
		if (ap.active_length == edgelength && next_rep_length > ap.active_length + ap_rep_length) {
			...
			Node copy_node = this.create_node(next.end - (edgelength + this.get_node_length(ap.active_node)) + 1,
					next.end, this.get_node_length(ap.active_node) + edgelength, next.stringnr);
			...
			create_edge_left(copy_node, next, this.get_letter(left_char_occ, next.stringnr)); // create left_edge from
																								// split to next
			create_edge_left(copy_node, sink, this.get_letter(pos - this.get_node_length(copy_node), sink.stringnr)); // create
																														// left_edge
																														// from
																														// split
																														// to
			...
			create_edge_left(copy_node.suffixLink, copy_node, this.get_letter(copy_node.start, copy_node.stringnr)); // create
																														// left_edge
																														// from
			...
			while (true) {
				...
				while (this.get_edge_length(ap.active_edge, ap.active_node, next)
					...
					if (this.get_edge_length(ap.active_edge, ap.active_node, copy_node)
							+ this.get_node_length(ap.active_node) < this.get_node_length(copy_node)) {
						// länge start knoten + läng der kante < länge des zielknotens
						int left_letter_pos = copy_node.start + this.get_node_length(copy_node)
								- this.get_edge_length(ap.active_edge, ap.active_node, copy_node)
								- this.get_node_length(ap.active_node) - 1;
