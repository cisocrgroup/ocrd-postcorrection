...
public class Common_SCDAWG_Functions {
	 ...
	 *
	 * @param node: node from which search will be started
	 **********************************************************************************/

	public static HashSet<Integer> find_n_transitions_to_sinks(Node node,
			Online_CDAWG_sym scdawg, HashSet<Integer> acc) {

		Iterator<Map.Entry<Integer, Node>> it = node.children.entrySet()
				.iterator();
		while (it.hasNext()) {
			...
			for (int j = 0; j < scdawg.sinks.size(); j++) {
				if (scdawg.sinks.get(j) == child) {
					...
					for (int k = 0; k < scdawg.sinks.get(j).stringnumbers
							.size(); k++) {
		...
		Iterator<Map.Entry<Integer, Node>> it2 = node.children_left.entrySet().iterator();

		while (it2.hasNext()) {

			Map.Entry<Integer, Node> pair = it2.next();
		...
		Iterator<Map.Entry<Integer, Node>> it3 = node.children.entrySet().iterator();

		while (it3.hasNext()) {

			Map.Entry<Integer, Node> pair = it3.next();
			Node child = pair.getValue();
			find_n_transitions_to_sinks(child, scdawg, acc);
		}

		Iterator<Map.Entry<Integer, Node>> it4 = node.children_left.entrySet().iterator();

		while (it4.hasNext()) {
			Map.Entry<Integer, Node> pair = it4.next();
			Node child = pair.getValue();
			find_n_transitions_to_sinks(child, scdawg, acc);
		}
		return acc;
	 ...
	 * count_nodes()
	 *
	 * counts all paths for alle nodes in the SCDAWG left and right transitions
	...
	public HashMap<Node, Integer> count_nodes() {
		...
		scdawg.eachNode_BFS(scdawg.root, false, true,
				new Online_CDAWG_sym.Visitor() {
					public void visit(Node n) {

						Iterator<Map.Entry<Integer, Node>> it = n.children.entrySet().iterator();
						...
						Iterator<Map.Entry<Integer, Node>> it2 = n.children_left.entrySet().iterator();
	...
	public ArrayList<Endpos_Pair> get_quasi_maximal_nodes_pairwise() {
		...
		mainloop: for (int i = 0; i < scdawg.all_nodes.size(); i++) {
			...
			ArrayList<Integer> endpos_s1 = new ArrayList<>();
			ArrayList<Integer> endpos_s2 = new ArrayList<>();
			...
			while (it.hasNext()) {
				...
				if (n2.is_endNode) {
					int start_in_sink = e.pos - scdawg.get_node_length(node)
							- 1;

					if (start_in_sink > -1) {
						// Logger.debug(scdawg.get_node_label(node)+"
						// "+scdawg.get_node_label(n2)+"
						// :"+scdawg.get_node_length(node)+" "+e.pos+"
						// "+start_in_sink);

						int start_letter = scdawg.get_letter(start_in_sink,
								n2.stringnr);
...
//			if (sinkshit.size() == 2 && (endpos_s1.size() == 1 && endpos_s2.size() == 1)) {
			...
			// check for end node
			if (node.children_left.size() == 2
					&& scdawg.get_node_label(node).endsWith("$")) {
				sinkshit = new HashSet<>();
				for (Map.Entry<Integer, Node> e : node.children_left
						.entrySet()) {
					if (e.getValue().is_endNode) {
						sinkshit.add(e.getValue().stringnr);
						if (e.getValue().stringnr == 0) {
							if (!endpos_s1.contains(
									scdawg.stringset.get(0).length())) {
								endpos_s1.add(scdawg.stringset.get(0).length());
							}
						} else {
							if (!endpos_s2.contains(
									scdawg.stringset.get(1).length())) {
	...
	public void get_distinct_quasimaximal_nodes() {
		...
		scdawg.eachNode_BFS(scdawg.root, false, false,
				new Online_CDAWG_sym.Visitor() {
					public void visit(Node n) {
		...
		HashMap distinct_nodes_sorted = Util.sortByValues(distinct_nodes,
				"DESC");
	...
	public int[] get_string_occurences() {
		...
		scdawg.eachNode_DFS(scdawg.root, true, true,
				new Online_CDAWG_sym.Visitor() {
	 ...
	 *
	 * @param marked_nodes: Result from mark_children_of_single_occ_nodes()
	 **********************************************************************************/

	public HashMap<Node, Boolean> mark_children_of_single_occ_nodes(
			int[] marked_nodes) {

		HashMap<Node, Boolean> result = new HashMap();
		scdawg.eachNode_DFS(scdawg.root, true, false,
				new Online_CDAWG_sym.Visitor() {
...
//					}
	...
	public HashMap<Node, Integer> get_quasiminimal_nodes() {
		...
		int[] marked_nodes = this.get_string_occurences();
		HashMap<Node, Boolean> marked_nodes2 = this
				.mark_children_of_single_occ_nodes(marked_nodes);
		...
		HashMap<Node, Integer> right_edges_count = this
				.count_quasiminimal_nodes(marked_nodes);
	 ...
	 *
	 *
	 * @param marked_nodes: Result from get_string_occurences()
	 **********************************************************************************/
	...
	public HashMap<Node, Integer> get_quasimaximal_nodes(
			HashMap<Node, Boolean> marked_nodes) {
		...
		scdawg.eachNode_DFS(scdawg.root, true, false,
				new Online_CDAWG_sym.Visitor() {
					...
					public void visit(Node n) {
						...
						// scdawg.get_node_label(quasi_maximal_candidate));
						// else
						...
						if (marked_nodes.get(n)) {
							...
							while (it2.hasNext()) {
								...
								if (quasi_maximal_nodes_help.containsKey(n2)
										&& quasi_maximal_candidate != null) {

									// Logger.debug(" candidate: " +
									// quasi_maximal_candidate.children.entrySet().iterator().hasNext());

									Iterator it3 = quasi_maximal_candidate.children
											.entrySet().iterator();
									int sinkshit_candidate = 0;

									while (it3.hasNext()) {
										Map.Entry pair3 = (Map.Entry) it3
												.next();
										Node n3 = (Node) pair3.getValue();
									...
									int sinkshit_total = sinkshit_candidate
											+ sinkshit_n2;

									if (sinkshit_total == scdawg.sinks.size()) {
										// Logger.debug("NIMM IHN");
										quasi_maximal_nodes.put(
												quasi_maximal_candidate, count);
										quasi_maximal_candidate = null;
							...
							// Logger.debug(sinkset.size() + " " +
							// scdawg.sinks.size());

							if (sinkset.size() == scdawg.sinks.size()) { // wenn
																			// in
																			// allen
																			// sinks
																			// vorkommt
								...
								quasi_maximal_nodes.put(quasi_maximal_candidate,
										count);
								quasi_maximal_candidate = null;
								sinkset = new HashSet<Integer>();
		...
		HashMap<Node, Integer> result = Util.sortByValues(quasi_maximal_nodes,
				"DESC");
