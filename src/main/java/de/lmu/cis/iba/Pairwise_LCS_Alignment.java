...
public class Pairwise_LCS_Alignment {
	...
	ArrayList<ArrayList<LCS_Triple>> longest_common_subsequences = new ArrayList<ArrayList<LCS_Triple>>();
	...
	public void align() {
		...
		for (Endpos_Pair pair : quasi_max_nodes) {
			longest_common_subsequences.add(calculate_LCS(pair.nodes_in_s1, pair.nodes_endpos_s2));
		}
	}
	...
	public void align_new() {
		HashSet<Node> candidates = find_quasi_max_nodes();

		new HashMap<Node, Integer>();

		ArrayList<NavigableMap<Integer, Node>> filtered_candidates = filter_quasi_max_nodes(candidates);
		ArrayList<NavigableMap<Integer, Node>> reduced_candidates = remove_included_substrings(filtered_candidates);
		...
		for (int i = 0; i < reduced_candidates.size(); i++) {
			for (Map.Entry<Integer, Node> entry : reduced_candidates.get(i).entrySet()) {
				...
				scdawg.get_node_length(currentNode);
				entry.getKey();
				Logger.debug(scdawg.get_node_label(entry.getValue()) + " " + entry.getKey() + " "
	...
	public ArrayList<LCS_Triple> calculate_LCS(Node[] nodes_in_s1, HashMap<Node, ArrayList> nodes_endpos_s2) {
		...
		for (int i = 0; i < nodes_in_s1.length; i++) {
			if (nodes_in_s1[i] != null) {
				...
				for (int j = nodes_endpos_s2.get(nodes_in_s1[i]).size() - 1; j >= 0; j--) { // iterieren über alle
																							// dec sequences ;//
																							// HIER EIN TRIPEL! //
	...
	// *******************************************************************************
	public String LCS_to_JSONString() {
		return new Gson().toJson(getAligmentPairs());
	}
