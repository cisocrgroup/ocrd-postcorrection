			 ...
			 while (it.hasNext()) {
			      ...
			      Node child = (Node) pair.getValue();
			    	  ...
			    	  if (scdawg.sinks.get(j)==child) { // wenn rechtsübergang auf sink
	    	    		  ...
	    	    		  if (!longest_quasi_nodes.containsKey(scdawg.sinks.get(j).stringnr)) {
	   			    		  longest_quasi_nodes.put(scdawg.sinks.get(j).stringnr, scdawg.get_node_label(node));

	   			    	  }
	   			    	  else {
	   			    		  if (scdawg.get_node_length(node)>longest_quasi_nodes.get(scdawg.sinks.get(j).stringnr).toString().length()) {
   	   			    	   longest_quasi_nodes.put(scdawg.sinks.get(j).stringnr, scdawg.get_node_label(node));
	   			    		  }
	   			    	  }
	    	    		  ...
	    	    		  Iterator it2 = node.children_left.entrySet().iterator();
	    	   			  ...
	    	   			  while (it2.hasNext()) {
	    	   			      ...
	    	   			      if(scdawg.sinks.get(j) == child_left) { // wenn linksübergang auf sink
