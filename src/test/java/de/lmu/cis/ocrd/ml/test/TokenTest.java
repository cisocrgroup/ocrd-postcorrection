...
public class TokenTest {
	...
	public void testWithFirstOtherOCR() {
		...
		assertThat(Token.create("a", 1).addOCR(Word.create("b")).getOtherOCRAt(0).toString(), is("b"));
	}

	@Test
	public void testWithSecondOtherOCR() {
		assertThat(Token.create("a", 1).addOCR(Word.create("b")).addOCR(Word.create("c")).getNumberOfOtherOCRs(), is(2));
		assertThat(Token.create("a", 1).addOCR(Word.create("b")).addOCR(Word.create("c")).getOtherOCRAt(0).toString(), is("b"));
		assertThat(Token.create("a", 1).addOCR(Word.create("b")).addOCR(Word.create("c")).getOtherOCRAt(1).toString(), is("c"));
	}
}
