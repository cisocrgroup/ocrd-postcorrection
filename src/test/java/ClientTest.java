...
public class ClientTest {
	private static final String resourceAbbyyZip = "src/test/resources/1841-DieGrenzboten-abbyy-small.zip";
	private static final String resourceTesseractZip = "src/test/resources/1841-DieGrenzboten-tesseract-small.zip";
	private static final String resourceOcropusZip = "src/test/resources/1841-DieGrenzboten-ocropus-small.zip";
	private Client client;
	private Project project;

	@Before
	public void login() throws Exception {
		this.client = Client.login(Config.getInstance().getPocowebURL(), Config.getInstance().getPocowebUser(),
	...
	public void TestCreateNewProject() throws Exception {
		...
		assertThat(project.getProjectId(), is(project.getBooks().get(0).getProjectId()));
		...
		assertThat(book.getProjectId(), is(not(project.getBooks().get(1).getProjectId())));
		assertThat(project.getProjectId(), is(not(project.getBooks().get(1).getProjectId())));
		...
		assertThat(book.getProjectId(), is(not(project.getBooks().get(2).getProjectId())));
		assertThat(project.getProjectId(), is(not(project.getBooks().get(2).getProjectId())));
